...
class ReceiveNotification extends dbConnect
{
        ...
        if ($result->num_rows > 0) {
            ...
            if ($hasSale <= 0) {
                ...
                if ($stmt->affected_rows) {
                    ...
                    if ($Payload['status'] > 2) {
                        $this->verify(); // * Retorna ao inicio da função
                    }
            ...
            if ((intval($Payload['status']) == 3 || intval($Payload['status']) == 4) && intval($Purshcase_InternalStatus) < 3) {
                ...
                foreach ($products as $product) {
                    ...
                    $prodsMargin[] = $rowP['price'] / ($rowP['cost'] + 0.4 + ($rowP['price'] * 0.05));
                ...
                $saleMargin = (array_sum($prodsMargin) / count($prodsMargin)) - 1;

                //? Status 3 == Pagamento Aprovado, Produtos Vendidos
                $stmt = $mysqli->prepare("UPDATE vendas SET internalStatus = 3, paymentDate = ?, margin = ? WHERE reference = ?");
                $stmt->bind_param("sss", $Payload['lastEventDate'], $saleMargin,  $Payload['reference']);
                $stmt->execute();
                ...
                //> Atualiza Referencia de Compra do Cliente
                $stmt = $mysqli->prepare("SELECT purchases from client WHERE id = ?");
                $stmt->bind_param("s", $Payload['clientId']);
                $stmt->execute();
                $result = $stmt->get_result();
                $row = $result->fetch_assoc();
                $actPurchases = json_decode($row['purchases']) ?? [];
                $actPurchases[] = $Payload['reference'];
                ...
                $stmt->execute();
                $stmt->close();
            } //>Fim de Pagamento Aprovado



            //x Compra Devolvida
            //x ["status" = 6, "Value" : "Devolvido"], ["internalStatus" = 8, "Discount from Next Relatory"]
            if (intval($Payload['status']) == 6 && intval($Purshcase_InternalStatus) == 4) {
                ...
                if (!$stmt->execute()) {
                    $errorMessage = (array(
                        ...
                        'errorCode' => 521,
                        'code' => $this->notificationCode_,
                        "message" => "Erro ao Contabilizar Compra Devolvida",
                    )
                    );
                    $stmt->close();
                    $this->error_log("error", $errorMessage);
                ...
                $stmt->close();
            } else if (intval($Payload['status']) == 6 && intval($Purshcase_InternalStatus) == 3) {
                //! Compra Devolvida
                //! ["status" = 6, "Value" : "Devolvido"], ["internalStatus" = 9, "Not Need be Discounted"]
                $stmt = $mysqli->prepare("UPDATE vendas SET internalStatus = 6 WHERE reference = ?");
                $stmt->bind_param("s", $Payload['reference']); //? Status 6 == Pedido Devolvido
                $htmlJsonResponse['status'] = 201;
    ...
    private function sendMail($notification)
    {
        ...
        } else if ($model['status'] >= 100 && $model['status'] < 200) {
            return ((array("status" => 100, "message" => "mail Ok!")));
        } else {
            $errorMessage = (array(
                'status' => 400,
                'errorCode' => 400,
                'code' => $this->notificationCode_,
                'mailError' => $model['status'],
                "message" => "Erro Ao Enviar Email",
            )
            );
            $this->error_log("error", $errorMessage);
...
if (!empty($_POST['notificationCode']) || 1) {

    $notificationCode = $_POST['notificationCode'];
    $notificationCode =  "BAC47AAB107C107C792EE46B2FA8BFA6D8DD"; // 3 id = 30
